...
- Addresses preserve line 1, optional line 2, postal code, city, and an uppercase two-letter country code. A French address requires a five-digit postal code. Country-code checks are syntactic and do not assert membership in an external registry.
- SIREN is nine ASCII digits with a local Luhn checksum. SIRET is fourteen ASCII digits, begins with the profile SIREN, and uses the local checksum including the documented La Poste exception. A French VAT number is `FR`, a two-character uppercase alphanumeric key, and the same nine-digit SIREN; the deterministic numeric-key formula is checked when the key is numeric. These validations do not prove that an organization is registered, active, or VAT liable.
- Profile creation and full replacement are explicit operations under the owner workspace. Updating a profile does not update any InvoiceDraft revision, artifact, approval, or delivery.
...
- An `IssuedInvoiceArtifact` is an immutable representation of exactly one persisted `IssuedInvoice`, and is a separate identity from the existing draft-bound `InvoiceArtifact`.
- Rendering consumes only the reconstructed immutable issuance snapshot. It never reads mutable billing profiles/settings, draft content, rates, TimeEntries, or Client display identity and never recalculates financial or date facts.
- `pdf` is the first representation type. Representation type and an explicit application-owned renderer version are part of canonical identity so later PDF versions or Factur-X representations do not alter historical artifacts.
