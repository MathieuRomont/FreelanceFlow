...
| RateAgreement | Hourly Decimal amount, currency, client or project scope, effective start/end dates | Project overrides client; missing or conflicting applicable rates block billing; effective-date interpretation must be explicit |
| Invoice | Billing period, currency, issuer/client snapshots, content version, totals, lifecycle information, artifact reference | One currency; versioned approved content; edits invalidate approval; sent content is immutable |
| InvoiceLine | Description, exact quantity, applied hourly rate, amount, and tax data where supported | Belongs to a specific invoice content version; snapshots calculations rather than reading mutable current rates |
